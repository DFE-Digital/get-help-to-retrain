...
class JobProfileDecoratorV2 < SimpleDelegator # rubocop:disable Metrics/ClassLength
  ...
  # SECTIONS_XPATH = {
  #   apprenticeship: content['HowToBecome']['EntryRoutes']['Apprenticeship']['RelevantSubjects'].join(", "),
  #   direct_application: content['HowToBecome']['EntryRoutes']['DirectApplication'].join(", "),
  #   work: content['HowToBecome']['EntryRoutes']['Work'].join(', '),
  #   day_to_day_tasks: content['WhatYouWillDo']['WYDDayToDayTasks'].first,
  #   working_environment: content['WhatYouWillDo']['WorkingEnvironment']['Location'],
  #   career_path: content['CareerPathAndProgression']['CareerPathAndProgression'].join(', '),
  #   restrictions_and_requirements: content['WhatItTakes']['RestrictionsAndRequirements']['OtherRequirements'].join(', '),
  #   career_tips: content['HowToBecome']['MoreInformation']['CareerTips'].join(', '),
  #   other_routes: content['HowToBecome']['EntryRoutes']['OtherRoutes'].join(', ')
  # }.freeze
  ...
  def how_to_become
    @how_to_become ||= "How to become an #{name}"
  end
  ...
  def section(key)
    ...
    s = ("<h1>#{key.to_s.humanize}</h1>" + xpath).html_safe
    separator_line.concat(s)
  end
