...
RSpec.describe AdminUser do
  ...
  describe '#roles' do
    ...
    it 'returns roles if multiple roles given' do
      admin_user.roles = %w[readwrite management]
      expect(admin_user.roles).to contain_exactly('readwrite', 'management')
    end
    ...
    it 'overwrites roles if already set' do
      admin_user.roles = %w[readwrite management]
      admin_user.roles = %w[read management]

      expect(admin_user.roles).to contain_exactly('read', 'management')
    end
    ...
    it 'ignores non existing roles' do
      admin_user.roles = %w[captain management]
      expect(admin_user.roles).to contain_exactly('management')
    end
  end
  ...
  describe '.with_roles' do
    it 'returns all admin users with a set role' do
      create(:admin_user, roles: ['management'])
      admin2 = create(:admin_user, roles: ['readwrite'])
      admin3 = create(:admin_user, roles: %w[management readwrite])

      expect(described_class.with_role('readwrite')).to contain_exactly(admin2, admin3)
    end

    it 'returns no admin users if role is not present' do
      create(:admin_user, roles: ['management'])
      expect(described_class.with_role('readwrite')).to be_empty
    end
  end
end
